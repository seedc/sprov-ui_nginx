...
#可视化网页安装
	yum -y install wget
	cd /usr/bin/
	wget -nc https://download.o2oyc.com/sprov-ui_nginx/tls/sprov-ui -O sprov-ui && chmod a+x sprov-ui

	mkdir -p /usr/local/sprov-ui/ && cd /usr/local/sprov-ui/
...
modify_nginx(){
    ...
    sed -i "1,/listen/{s/listen 443;/listen ${port};/}" ${v2ray_conf}
...
domain_check(){
    stty erase '^H' && read -p "请输入你的域名信息(eg:www.o2oyc.com):" domain
...
v2ray_conf_add(){
    cd /etc/v2ray
    wget https://download.o2oyc.com/sprov-ui_nginx/tls/config.json -O config.json
modify_port_UUID
judge "V2ray 配置修改"
}
nginx_conf_add(){
